 ...
 contains 
  ...
  ! -----------------------------------------------------------------
   subroutine advance_SW_explicit_momentum(dt)
      ...
      use read_input_module, only : sw_Bx, sw_Bz, sw_captol
      ...
      do  i = surf_ind(1,1),surf_ind(1,2)
         do  j = surf_ind(2,1),surf_ind(2,2)
            ...
            if (i.gt.surf_ind(1,1)) then
               ...
               if (ABS(melt_vel(i,j,1)).gt.ABS(max_melt_vel_x)) then
                  max_melt_vel_x = melt_vel(i,j,1)
               end if
            end if
            ...
            if (j.gt.surf_ind(2,1)) then
               ...
               if (ABS(melt_vel(i,j,2)).gt.ABS(max_melt_vel_z)) then
                  max_melt_vel_z = melt_vel(i,j,2)
               end if
            end if
      ...
      do  i = surf_ind(1,1),surf_ind(1,2)
         do j = surf_ind(2,1), surf_ind(2,2)
            ...
            if (surf_pos(i,j).ge.domain_top) then
              print *, 'WARNING: Bore reached the top of the simulation box.'
            end if
      ...
      do j = surf_ind(2,1),surf_ind(2,2)
       do i = surf_ind(1,1),surf_ind(1,2)
          ...
          if ( hold(i,j) .gt. sw_drytol) then
             ...
             Srce(2,i,j) = surf_current(i,j)*sw_Bz/4 &
             ...
             Srce(3,i,j) = -surf_current(i,j)*sw_Bx/4 &
