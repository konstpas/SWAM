module read_files_module
   ...
   contains
    ...
    ! -----------------------------------------------------------------
    subroutine read_heatflux_file(input_filename, length, tpoints, &
        ...
        real(amrex_real), allocatable, dimension(:), intent(out)  :: zpoints
     ...
     ! ------------------------------------------------------------------------
     ! Subroutine used to read the free surface position described in an input.
     ! It also returns arrays for the vectors that generated the free surface
     ! mesh in the input file.
     ! ------------------------------------------------------------------------
     subroutine read_free_surface_file(input_filename, length, points, &
                                       x_file, y_file, z_file)
 
         ! Input and output variables
         integer, intent(in) :: length
         character ( len = length ), intent(in) :: input_filename 
         integer, intent(in) :: points(1:2)
         real(amrex_real), intent(out) :: x_file(1:points(1))
         real(amrex_real), intent(out) :: y_file(1:points(1)*points(2))
         real(amrex_real), intent(out) :: z_file(1:points(2))
         ! Local variables
         logical got_one
         character ( len = 255 ) line
         integer :: dims(1:2)
         ...
         do while (.not.got_one) ! Go through the document until you find a non
             ...
             got_one = .true. ! The dimensions of the mesh ("dims") have been found.
             if (dims(1).ne.points(1) .or. dims(2).ne.points(2) ) then
                STOP 'Format of free surface file not consistent'
             end if
         end do
 
         call r8vec_data_read (input_unit, input_status, points(1), x_file) ! Reads points amount of lines and finds the 
                                                                         ! vector that describes the time coordinates
 
         call r8vec_data_read (input_unit, input_status, points(2), z_file) ! Reads points amount of lines and finds the 
                                                                         ! vector that describes the z-coordinates

         call r8vec_data_read (input_unit, input_status, points(1)*points(2), y_file) ! Reads points amount of lines and finds the 
                                                                         ! vector that describes the x-coordinates
 
     
         close ( unit = input_unit )
     end subroutine read_free_surface_file
     ...
     ! -----------------------------------------------------------------
     subroutine get_mesh_dimensions(input_filename, dims, length, points)
         ...
         do while (.not.got_one)! Go through the document until you find a non
             ...
             call s_to_i4vec ( line, dims, points, ierror)    
             if ( ierror /= 0 ) then
                 cycle
             end if
             got_one = .true.  ! The dimensions of the mesh ("points") have been found.
         end do
