...
!$omp parallel private(mfi,bx,tbx,pin,pout)
    ...
    do while(mfi%next())
       bx = mfi%tilebox()
       pin  => phi_old(lev)%dataptr(mfi)
       pout => phi_new(lev)%dataptr(mfi)
       !lowbound = lbound(pin) 
       !hbound = ubound(pin)
       

	do i=lbound(pin,1),ubound(pin,1) 
		do j = lbound(pin,2),ubound(pin,2) 
		pout(i,j,1,1) = 1._amrex_real  !! why the extra dimensions if spacedim = 2 etc. 
		end do 
	end do 
