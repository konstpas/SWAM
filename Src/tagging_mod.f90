...
contains
  ...
  subroutine tag_phi_error (level, time, lo, hi, xlo, dx, surfpos, surfdist, phi, philo, phihi, tag, taglo, taghi, phierr, &
       settag, cleartag)  
    ...
    real(amrex_real)      , intent(in   ) :: surfpos, xlo(3), dx(3)  ! Surface y-position (to be multifab on DIM-1) 
    ...
    do       k = lo(3), hi(3)
       do    j = lo(2), hi(2)
          do i = lo(1), hi(1)
             ...
             ydist = abs(xlo(2) + (j-lo(2))*dx(2) - surfpos ) 
