module read_files_module
  ...
  subroutine read_free_surface_file(input_filename, length, points, &
     ...
     real(amrex_real), intent(out) :: x_file(1:points)
     real(amrex_real), intent(out) :: y_file(1:points)
     ! Local variables
     logical got_one
     character ( len = 255 ) line
     integer :: dims(1:1)
     ...
     do while (.not.got_one) ! Go through the document until you find a non
           ...
           call s_to_i4vec ( line, 1, dims, ierror)   
           if ( ierror /= 0 ) then
              cycle
           end if
           got_one = .true. ! The dimensions of the mesh ("dims") have been found.
           if (dims(1).ne.points ) then
              STOP 'Format of free surface file not consistent'
           end if
     end do

     call r8vec_data_read (input_unit, input_status, points, x_file) ! Reads points amount of lines and finds the 
                                                                     ! vector that describes the time coordinates

     call r8vec_data_read (input_unit, input_status, points, y_file) ! Reads points amount of lines and finds the 
