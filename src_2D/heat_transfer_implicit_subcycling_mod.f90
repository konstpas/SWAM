module heat_transfer_implicit_subcycling_module
...
contains
  ...
  subroutine advance_heat_solver_implicit_level(lev, time, dt, substep, &
    ...
    call init_tmp_multifab(lev, time, phi_tmp, temp_tmp, idomain_tmp)
    ...
    call advance_advection(lev, dt, substep, temp_tmp, phi_tmp)
  ...
  ! -----------------------------------------------------------------
  subroutine init_tmp_multifab(lev, time, phi_tmp, temp_tmp, idomain_tmp)
  ...
  subroutine advance_conduction(lev, time, dt, phi_tmp, temp_tmp, &
    ...
    call conduction_synch_idomain(lev, temp_tmp)
  ...
  ! of the heat transfer equation
  ! -----------------------------------------------------------------
  subroutine advance_advection(lev, dt, substep, temp_tmp, phi_tmp)
    ...
    !$omp parallel private(mfi, bx, pidom, ptempin, ptemp, pin, pout, tbx, flux, pfx, pfy)
    ...
    do while(mfi%next())
       ...
       do idim = 1, amrex_spacedim
          ...
          call flux(idim)%resize(tbx,phi_new(lev)%ncomp())
          call tbx%grow(substep)
       end do
       pfx => flux(1)%dataptr()
       pfy => flux(2)%dataptr()
       call advection_box(bx%lo, bx%hi, &
  ...
  subroutine conduction_correct(lev, phi_tmp, temp_tmp, alpha)
    ...
    ! Update ghost points in the enthalpy multifab
    call phi_tmp%copy(phi_new(lev), 1, 1, 1, 1)
    call phi_tmp%fill_boundary(geom)
    
    ! Update ghost points in the temperature multifab
    call temp_tmp%fill_boundary(geom)
  ...
  ! part of the heat transfer equation
  ! -----------------------------------------------------------------
  subroutine conduction_synch_idomain(lev, temp_tmp)
  ...
  subroutine get_enthalpy_conduction(lo, hi, &
    ...
    ! Obtain enthalpy from temperature
    call get_temp(un_lo, un_hi, &
                  u_new, un_lo, un_hi, &
                  temp_new, tn_lo, tn_hi, .false.)
    
    ! Update enthalpy for state points undergoing phase-transition
    do j = lo(2), hi(2)
       do i = lo(1), hi(1)
    ...
    ! Update temperature with ghost points
    call get_temp(lo, hi, &
                  u_new, un_lo, un_hi, &
                  temp_new, tn_lo, tn_hi, .true.)
    
    ! Update temperature without ghost points
    call get_temp(lo, hi, &
                  u_new, un_lo, un_hi, &
  ...
  ! -----------------------------------------------------------------  
  subroutine advection_box(lo, hi, &
                           u_old,  uo_lo, uo_hi, &
                           u_new, un_lo, un_hi, &
                           temp, t_lo, t_hi, &
                           flxx, fx_lo, fx_hi, &
    ...
    do  j = lo(2),hi(2)
       do i = lo(1),hi(1)
          u_new(i,j) = u_old(i,j) &
                       - dt/dx(1) * (flxx(i+1,j) - flxx(i,j)) &  
                       - dt/dx(2) * (flxy(i,j+1) - flxy(i,j)) & 
                       + dt*qvol(i,j)
       end do
    end do
...
end module heat_transfer_implicit_subcycling_module
